 ...
 * @return string         属性信息
 */
function get_model_attribute($model_id, $group = true,$fields=true,$no_field=""){
    static $list;
    ...
    if(!isset($list[$model_id])){
        ...
        /*BY.jingshuixian   添加屏蔽字段*/
        if(!empty($no_field)){
            $map[] = array('name'=> array("not in", $no_field));
        }
...
function forma_list_data($list ,$type = 'cate',$attr = array()){
    ...
    foreach($list as $key => $item){
        if(LONG_IIS6){  //iis6 的url模式
            ...
            $list[$key] =format_item_data_no_rewrite($item,$type ,$attr);
        }else{
...
function format_item_data($item ,$type = 'cate',$attr = array()){
    ...
    if(LONG_IIS6){
        return format_item_data_no_rewrite($item,$type ,$attr);
    }
    ...
    if($type == 'cate') {
        ...
        /*By.jingshuixian  2017年1月23日 设置当前栏目 active 样式*/
        $caid = I('category');
        $item['class'] =   $caid == $item['id'] ? 'active':'';
        ...
        if($item['link_id'] > 0){
            $item['link_url']  = get_link($item['link_id']);
        }
 ...
 * @return mixed
 */
function format_item_data_no_rewrite($item ,$type = 'cate',$attr = array()){
    ...
    if($type == 'cate') {
        ...
        if ($item['type'] == 4) {
            $href = get_nav_url('Home/Article/page?category=' . (empty($item['name'])?$item['id']:$item['name']));
        } elseif ($item['type'] == 5) {
            $url_arr = str2arr($item['url'],'/');


                if($url_arr[0]=='page'){
                    $href = get_nav_url('Home/Article/page?category=' . $url_arr[1]);
                }elseif($url_arr=='lists'){
                    $href = get_nav_url('Home/Article/lists?category=' . $url_arr[1]);
            ...
            $href = get_nav_url('Home/Article/lists?category=' .  (empty($item['name'])?$item['id']:$item['name']));
        }
        $item['href']  = $href;
        ...
        $href = get_nav_url('Home/Article/detail?id=' . (empty($item['name'])?$item['id']:$item['name']));
...
function get_pc_logo(){
    return get_cover(C('WEB_LOGO'),'path');
}

function get_m_logo(){
    return get_cover(C('COM_M_LOGO'),'path');
}
...
function html2text($content = '',$sublength=0,$dot='...'){
    if(!empty($content)){
        $content = strip_tags($content) ;
    }
    $content = trim($content);
    if($sublength > 0){
        $content = _substr($content,$sublength,$dot);
    }
    return $content ;
}
...
function get_filter($category_id=0 , $model_id =0 ,$active = 'active'){
    ...
    foreach($attr as $key=>$value){
        ...
        $attrurl = array();
        $attrurl[] = LONG_IIS6 ? 'Home/Article/lists/category': '/lists';
        $attrurl[] = $category_id;
...
function get_filter2($model_id =0 ,$active = 'active',$all_title = '全部',$add_where = true){
    ...
    foreach($attr as $key=>$value){
        ...
        //判断是否给全部加样式
        $link['title']= $all_title;
        $link['class']= empty($val)?$active:'';
        $link['href'] =  '';
        $link['vars'] =  $vars;
        $links[] = $link;  //添加到全部
        ...
        //添加条件
        if(!empty($val)){
            if(is_array($val)){  // 如果获得的值是数组集合
                $titles = get_array_val($val,$items);

            }else{
                $link['title']= $title . ':' . $items[$val];
            }
        ...
        foreach($items as $k=>$v){
            $link = array();
            $link['title'] = $v;
            $link['vars'] = array_merge($vars,array($name=>$k));
    ...
    //添加条件
    if($add_where){
        $returnArrayOne['title'] = "条件";
        $returnArrayOne['name'] = "where";
        $returnArrayOne['_'] = $where;
        array_unshift($returnArray,$returnArrayOne) ;
    }
    ...
    $map = get_filter_maps($attr);

    return array($returnArray,$map);
}
...
function get_em($model_id ,$filed ='', $data=''){
    ...
    //By.jingshuixian 删除 $data 的空判断  if(!empty($attr) and !empty($data)){
    if(!empty($attr)){
        $str = explode(',',$data);
        $extra = $attr['extra'];
        $items = parse_field_attr($extra);
...
function get_em_by_config($config,$data){
		$return = array();
