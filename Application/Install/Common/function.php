...
function check_dirfile(){
    $items = array(
        ...
        array('dir',  '可写', 'success', LONG_WEB_SITE_PATH.'/Uploads/Picture'),
        array('dir',  '可写', 'success', LONG_WEB_SITE_PATH.'/Uploads/Editor'),
        array('dir',  '可写', 'success', LONG_WEB_SITE_PATH.'/Uploads/Attachment'),
        array('dir',  '可写', 'success', LONG_WEB_SITE_PATH.'/Uploads/Download'),
        array('dir',  '可写', 'success', RUNTIME_PATH),
        array('dir',  '可写', 'success', LONG_WEB_SITE_DATA_PATH),
    ...
    if(is_array($config)){
        ...
        if(!IS_WRITE){
           ...
           // <textarea name="" style="width:650px;height:125px">'.$user.'</textarea>';
            ...
            if(file_put_contents(APP_PATH . 'Common/Conf/config_'.LONG_WEB_SITE_NAME.'.php', $conf) ){
                show_msg('配置文件写入成功');
            } else {
...
function create_tables($db, $prefix = ''){
    ...
    //$sql = str_replace("default-ss", LONG_WEB_SITE_PATH, $sql,$i);
...
function update_tables($db, $prefix = ''){
    ...
    //替换数据库备份路劲
    //$sql = str_replace("./Data/default-ss", "./Data/".LONG_WEB_SITE_PATH, $sql,$i);
    //替换模板
   // $sql = str_replace("default-ss", LONG_WEB_SITE_PATH, $sql);
    //开始安装
    show_msg('开始升级数据库...');
    foreach ($sql as $value) {
        $value = trim($value);
        ...
        if(substr($value, 0, 12) == 'CREATE TABLE') {
            ...
            if(false !== $db->execute($value)){
                ...
                $sql = "UPDATE  `".$prefix."config` SET  `value` =  './Data/".LONG_WEB_SITE_PATH."/' WHERE  `".$prefix."config`.`id` =28";
                $db->execute($sql);
                show_msg($msg . '...成功');
            } else {
                show_msg($msg . '...失败！', 'error');
