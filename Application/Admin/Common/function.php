    ...
    if(!empty($grid['href'])){
        ...
        foreach($links as $link){
            ...
            if(preg_match('/^\[([a-z_]+)\]$/',$href,$matches)){
                $val[]  =   $data2[$matches[1]];
            }elseif(preg_match('/^\(([^\s]*)\)/',$href,$matches)){ //自定义链接
                $show   =   isset($array[1])?$array[1]:$value;
                $href = $matches[1];
                // 替换数据变量
                $href   =   preg_replace_callback('/\[([a-z_]+)\]/', function($match) use($data){return $data[$match[1]];}, $href);
                //BY.jingshuixian  2017年1月17日15:04:29  增加了第三个参数 样式    //think/lists/model/xingcheng/document_id/[id]/cate_id/[category_id]
                if(0 === strpos($href,'//')) {  //(//detail/[id].html)|预览|target_blank   增加预览打开方式  红专v3版本里使用
                    $href =   substr($href,1);
                    $val[]  =   '<a href="'.$href.'" class="'. $array[2] .'">'.$show.'</a>';
                }else{
                    $val[]  =   '<a href="'.U($href).'" class="'. $array[2] .'">'.$show.'</a>';
                }
                ...
                $show   =   isset($array[1])?$array[1]:$value;
                // 替换系统特殊字符串
                //By.jingshuixian  删除链接添加model 参数
                $href   =   str_replace(
                ...
                // 替换数据变量
                $href   =   preg_replace_callback('/\[([a-z_]+)\]/', function($match) use($data){return $data[$match[1]];}, $href);
                //BY.jingshuixian  2017年1月17日15:04:29  增加了第三个参数 样式
                $val[]  =   '<a href="'.U($href).'" class="'. $array[2] .'">'.$show.'</a>';
            }
        }
        $value  =   implode(' ',$val);
...
function get_attribute_type($type=''){
    ...
    static $_type = array(
        ...
        'pictures'   =>  array('上传图集','varchar(255) NOT NULL'),
        'file'      =>  array('上传附件','int(10) UNSIGNED NOT NULL'),
        'url'      =>  array('链接','int(10) UNSIGNED NOT NULL'),
        'baidumap'    =>  array('百度地图','varchar(999) NOT NULL'),
        'join'    =>  array('关联文档','varchar(50) NOT NULL'),
        //'bigfile' =>  array('上传文件','int(10) UNSIGNED NOT NULL'),
    );
...
//By.jingshuixian 移动到common 公用方法里
 ...
 * @param int $type
 * @return mixed
 * By.jingshuixian  2016年3月18日 23:42:11
 */
function get_attribute_show($type=0){
    // TODO 可以加入系统配置
    static $_type = array(
        0       =>  "不显示",
       1    =>  "始终显示",
