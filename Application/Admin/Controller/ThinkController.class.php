...
class ThinkController extends AdminController {
    ...
    public function lists($model = null, $p = 0){
        ...
        foreach($_REQUEST as $name=>$val){
            //By.jingshuixian 2017年1月19日10:51:32 删除过滤字段的功能
            //if(in_array($name,$fields)){
                $map[$name]	=	$val;
            //}
        }

        //By.jingshuixian  begin
        //增加状态条件判断  用俩执行删除 等操作
    ...
    public function setStatus($model='Document'){
        //By.jingshuixian
        if(is_numeric($model)){ //通过ID查询
            $model = get_model_name_by_id($model);
        }
        //echo $model;
        //By.jingshuixian  end
    ...
    public function edit($model = null, $id = 0){
        ...
        if(IS_POST){
            ...
            if($Model->create() && $Model->save()){
                //By.jingshuixian  2017年1月17日15:49:58  添加document_id绑定跳转
                $document_id = I('document_id',0);
                if($document_id){
                    $href = U('lists?model='.$model['name'].'&document_id='.$document_id);
                }else{
                    $href = U('lists?model='.$model['name']);
                }
                $this->success('保存'.$model['title'].'成功！', $href);

            } else {
                //By.jingshuixian  2017年1月17日15:49:58  添加错误空值判断
                $msg = $Model->getError() ;
                $msg = $msg ? $msg : '保存失败!';
                $this->error($msg);
            }
    ...
    public function add($model = null){
        ...
        if(IS_POST){
            ...
            if($Model->create() && $Model->add()){
                ...
                $this->success('添加'.$model['title'].'成功！', $href);
