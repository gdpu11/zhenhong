...
class Verify {
    ...
    public function entry($id = '') {
        ...
        // 图片高(px)
        $this->imageH || $this->imageH = $this->fontSize * 1.8;
        ...
        if($this->useZh){ // 中文验证码
            for ($i = 0; $i<$this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $this->fontSize*($i+1)*1.2, $this->fontSize + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
            }
        }else{
            for ($i = 0; $i<$this->length; $i++) {
     ...
     * @param string $code
     */
    public function addEntry($id = '',$code="") {
        $key        =   $this->authcode($this->seKey);
        $code       =   $this->authcode(strtoupper($code));
        $secode     =   array();
        $secode['verify_code'] = $code; // 把校验码保存到session
        $secode['verify_time'] = NOW_TIME;  // 验证码创建时间
        session($key.$id, $secode);
